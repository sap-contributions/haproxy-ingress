...
func (c *converter) trackAddedIngress() {
	for _, ing := range append(c.changed.IngressesAdd, c.changed.IngressesUpd...) {
		...
		for _, rule := range ing.Spec.Rules {
			...
			// A rules-based tcp service (spec.rules) carries its backend in
			// the TCPServiceHost, not via a resolvable HTTP path. findBackend
			// above re-derives the backend from the Service and may return nil,
			// so an annotation-only update (e.g. a changed config-backend)
