...
func TestSyncPartialTCPServiceAnnUpdate(t *testing.T) {
	...
	// spec.defaultBackend. The full parse tracks Ingress->HATCPService and
	// Ingress->HABackend links.
	c.createSvc1("default/echo1", "8080", "172.17.0.11")
	...
	// renders (silent staleness surfacing later as a broken backend).
...
func TestSyncPartialTCPServiceRulesAnnUpdate(t *testing.T) {
	...
	// Full sync: a TCP-service ingress whose backend is reached via
	// spec.rules[].http.paths[].backend with a host set. The TCP service
	...
	// Annotation-only partial sync: widen the acl. trackAddedIngress must
	...
	// skipped - the new config-backend never renders.
	ann["ingress.kubernetes.io/config-backend"] = "acl allow src 0.0.0.0/0"
	ing = c.createIng1Ann("default/echo1", "echo.local", "/", "echo1:8080", ann)
	c.cache.Changed.IngressesUpd = []*networking.Ingress{ing}
	c.Sync()
