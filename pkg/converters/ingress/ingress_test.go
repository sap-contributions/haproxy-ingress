...
func TestSyncPartialTCPServiceRulesAnnUpdate(t *testing.T) {
	c := setup(t)
	defer c.teardown()

	// Full sync: a TCP-service ingress whose backend is reached via
	// spec.rules[].http.paths[].backend with a host set (the shape the
	// hanaservice-operator actually emits for PrivateLink). The TCP service
	// registers the backend under the rule host, not under defaultbackend.
	c.createSvc1("default/echo1", "8080", "172.17.0.11")
	ann := map[string]string{
		"ingress.kubernetes.io/" + ingtypes.TCPTCPServicePort: "7001",
		"ingress.kubernetes.io/config-backend":                "acl network_allowed src 10.0.0.0/8",
	}
	ing := c.createIng1Ann("default/echo1", "echo.local", "/", "echo1:8080", ann)
	c.Sync(ing)
	c.hconfig.Commit()
	c.logger.Logging = []string{}
	...
	// Simulate a prior partial sync whose destructive QueryLinks cascade
	// consumed this ingress' tracker links without re-parsing the ingress.
	// This is the live-cluster precondition for the bug.
	c.tracker.ClearLinks()

	// Annotation-only partial sync: widen the network_allowed acl (the
	// whitelist-widen mutation the CI test performs). trackAddedIngress must
	// rebuild the links the previous sync consumed. For a rules-based TCP
	// ingress findBackend re-derives the backend from the Service and may
	// return nil, so without tracking the TCP service's existing backend the
	// resync yields an empty dirtyTCPServices set, the stale TCP service is
	// not removed, re-parse hits "already assigned", and the backend resync is
	// skipped - the new config-backend never renders (the -10709 failure).
	ann["ingress.kubernetes.io/config-backend"] = "acl network_allowed src 0.0.0.0/0"
	ing = c.createIng1Ann("default/echo1", "echo.local", "/", "echo1:8080", ann)
	c.cache.Changed.IngressesUpd = []*networking.Ingress{ing}
	c.Sync()
...
func (u *updaterMock) UpdateFrontConfig(front *hatypes.Frontend, mapper *annotations.Mapper, localPorts bool) {
}
